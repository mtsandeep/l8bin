...
### Changed
- **docker-observe proxy now pools its backend socket connections** — the managed HAProxy config sets `http-reuse always` with `pool-max-conn 16`, so observer clients that open a fresh connection per Docker API request no longer cause a new daemon socket connection per call. Existing proxies are recreated with the new config on their project's next start/wake/redeploy.
...
- **`l8b ship` asked "Direct to agent or relay via master?" when deploying to the local node** — both are the same there; the prompt is now skipped for the local node only.
...
- **`--upload` flag for `l8b deploy`** — Force the upload path in CI/scripts: `--upload direct` (client → agent), `--upload relay` (client → master → agent), or `--upload auto` (default; direct when the node supports it, else relay). Applies to both single-image and compose deploys.
...
- **Capability-grant prompt hidden behind spinner** — spinner is suspended during the interactive confirm.
- **Compose build failed on Windows** — `\\?\` verbatim prefix from `canonicalize()` is now stripped via `dunce`.
...
- **`${VAR:?msg}` tripped at deploy time** — Required-var check is now enforced only at container start (on the node, with its `.env` loaded); central validation is lenient since it has no access to runtime secrets.
